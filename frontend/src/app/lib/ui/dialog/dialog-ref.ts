...
import { ZardDialogComponent, ZardDialogOptions } from './dialog';
...
export class ZardDialogRef<T = any, R = any, U = any> {
  ...
  close(result?: R) {
    ...
    setTimeout(() => {
      ...
      // Emit the result and complete closed$ so callers can react to the final value
  ...
  /**
   * Returns an observable that emits the result passed to close() when the dialog finishes closing.
   */
  afterClosed() {
